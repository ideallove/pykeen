...
最新稳定版的PyKEEN版本需要Python 3.8+，可以通过[PyPI](https://pypi.org/project/pykeen)下载和安装：

```shell
pip install pykeen
```
直接从 [GitHub](https://github.com/pykeen/pykeen) 上下载安装最新版本的PyKEEN：

```shell
pip install git+https://github.com/pykeen/pykeen.git
```

更多关于安装的消息（例如development mode, Windows installation, Colab, Kaggle, extras）
可以在 [安装文档](https://pykeen.readthedocs.io/en/latest/installation.html) 查看
...
这个例子展示了如何在一个数据集上训练模型并在另一个数据集上测试。
...
最快的入门方法是使用pipeline函数。它提供了一个高层次的入口，可以扩展这个包的功能。
以下示例展示了如何在[Nations](https://pykeen.readthedocs.io/en/latest/api/pykeen.datasets.Nations.html#pykeen.datasets.Nations)数据集上训练和评估[TransE](https://pykeen.readthedocs.io/en/latest/api/pykeen.models.TransE.html#pykeen.models.TransE)模型。
默认情况下，训练循环使用[随机局部封闭世界假设（sLCWA）](https://pykeen.readthedocs.io/en/latest/reference/training.html#pykeen.training.SLCWATrainingLoop)训练方法，并使用[基于排名的评估](https://pykeen.readthedocs.io/en/latest/reference/evaluation/rank_based.html#pykeen.evaluation.RankBasedEvaluator)进行评估。
